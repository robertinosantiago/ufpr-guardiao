<h1><?php echo __('Níveis de acesso'); ?></h1>

<div class="breadcrumb">
    <?php $this->Html->addCrumb(__('Níveis de acesso'), '/Nivels'); ?>
    <?php
...
<?php echo $this->Form->create('Nivel', array('id' => 'formindex', 'action' => 'deleteMany')) ?>
        ...
        <?php else: ?>
            ...
            <tr>
                <td class="text-center">
                    <?php echo $this->Form->checkbox('Records.id.' . $record['Nivel']['id'] . '', array('label' => false, 'value' => $record['Nivel']['id'], 'class' => 'ids', 'hiddenField' => false)); ?>
                </td>
                <td class="text-left"><?php echo $record['Nivel']['titulo']; ?></td>
                <td class="text-center">
                    <?php echo $this->Form->postLink(__("<i class='fa fa-pencil'></i>"), array('action' => 'update', $record['Nivel']['id']), array('class' => 'btn btn-sm btn-warning', 'title' => __('Editar'), 'escape' => false)); ?>
                </td>
                <td class="text-center">                    
                    <?php echo $this->Form->postLink(__("<i class='fa fa-trash-o'></i>"), array('action' => 'delete', $record['Nivel']['id']), array('class' => 'btn btn-sm btn-danger', 'title' => __('Excluir'), 'escape' => false, 'confirm' => __('Você deseja realmente excluir?'))); ?>
                </td>
            </tr>
            <?php endforeach; ?>
        <?php endif; ?>
